...
```{r top artists by num songs on chart, fig.width = 8, fig.height = 8}
...
```{r top artists by No1 songs on chart, fig.width = 8, fig.height = 8}
...
```{r AG Uniq songs,  fig.width = 12, fig.height = 8}
...
```{r AG parcoords kpop, fig.width = 12, fig.height=7}
fulldata %>%
  mutate(year = as.numeric(format(first_week, format="%Y"))) %>%
  # mutate(year = as.numeric(format(first_week, format="%Y")),
